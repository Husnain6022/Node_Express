...
app.post("/jokes", (request, response) => {
  const newJoke = {
    id : jokes.length + 1,
    jokeText : request.body.text,
    jokeType : request.body.type
  }
  jokes.push(newJoke)
...
app.patch("/jokes/:id", (request, response) => {
  const id = parseInt(request.params.id)
  const oldJoke = jokes.find((joke) => joke.id === id)
  const newJoke = {
    id : id,
    jokeText : request.body.text || oldJoke.jokeText,
    jokeType : request.body.type || oldJoke.jokeType
  }
  const updatedJokeIndex = jokes.findIndex((joke) => joke.id === id)
  jokes[updatedJokeIndex] = newJoke
  response.json(jokes[updatedJokeIndex])
})
...
app.delete("/jokes/:id", (request, response) => {
 ...
 if (jokeIndex > -1){
  ...
  response.status(404)
  response.json({error: `joke with ${id} not found. No jokes were deleted.`})
 }
})
...
app.delete("/all", (request, response) => {
  ...
  if (userKey === masterKey){
    ...
    response.status(404)
    response.json({error: `You are not authourized to this action`})
  }
})
//8. DELETE All jokes
